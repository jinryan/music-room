...
export class AutonomousEngine {
  ...
  private currentComplexity: number = 0.5; // Store complexity to apply to new layers
  ...
  async start(): Promise<void> {
    ...
    try {
      ...
      // Apply current complexity setting to the new melody layer
      this.melodyLayer.setComplexity(this.currentComplexity);
      ...
      this.barInterval = (globalThis.setInterval || setInterval)(() => {
        ...
        // Schedule next loop to start AFTER current loop finishes (no overlap)
        if (this.barCounter % 32 === 0) {
          const currentTime = this.context.currentTime;
          const nextLoopStart = currentTime + 0.001; // Start after current loop finishes
  ...
  setMelodyComplexity(complexity: number): void {
    this.currentComplexity = complexity; // Store for when layer is recreated
    if (this.melodyLayer) {
      this.melodyLayer.setComplexity(complexity);
    } else {
      // Log if layer doesn't exist yet (will be applied when layer is created)
      console.log(`[AutonomousEngine] setMelodyComplexity(${complexity}) - layer not created yet, will apply on start`);
    }
  }
